...
import type { LLMProviderType } from "../types";
    ...
    export class Agent {
      ...
      /**
       * Constante: maximo de mensagens no historico antes de truncar
       */
      private readonly MAX_HISTORY = 50;

      /**
       * Trunca o historico se exceder MAX_HISTORY mensagens.
       * As mensagens removidas sao sumarizadas e salvas na memoria de longo prazo.
       */
      private async truncateHistoryWithSummary(): Promise<void> {
        if (this.history.length <= this.MAX_HISTORY) return;

        // Separa mensagens a serem removidas (exceto system e as ultimas N-1)
        const systemMsg = this.history[0];
        const keepCount = this.MAX_HISTORY - 1; // reserva espaco para system
        const messagesToKeep = this.history.slice(-keepCount);
        const messagesToRemove = this.history.slice(1, this.history.length - keepCount);

        if (messagesToRemove.length === 0) return;

        console.log(`[Agent] Truncando historico: ${messagesToRemove.length} mensagens serao sumarizadas.`);

        try {
          // Criar um resumo usando o proprio modelo
          const summaryText = messagesToRemove
            .filter(m => m.content && m.role !== "system")
            .map(m => {
              const prefix = m.role === "user" ? "Usuario" : m.role === "assistant" ? "Assistente" : "Ferramenta";
          ...
          if (summaryText.trim().length > 50) {
            ...
            const summaryResponse = await this.retryWithBackoff(
              () => this.client.chat.completions.create({
                ...
                messages: [
                  {
                    role: "system",
                    content: "Voce e um sumarizador de conversas. Resuma o dialogo abaixo em 2-3 frases concisas em portugues, destacando: topicos principais, decisoes tomadas, preferencias do usuario e informacoes importantes. Seja objetivo."
                  },
                  {
                    role: "user",
            ...
            const summary = (summaryResponse as any).choices[0].message.content || "";

            if (summary && summary.length > 10) {
              // Salvar na memoria de longo prazo via saveDailyNote
              try {
                const { saveDailyNote } = await import("../memory/memory_search");
                const noteContent = "## Contexto arquivado (" + this.name + ")\n- Data: " + new Date().toISOString().split("T")[0] + "\n- Resumo: " + summary.replace(/\n/g, " ").trim() + "\n- Mensagens arquivadas: " + messagesToRemove.length;
                saveDailyNote(noteContent);
                console.log("[Agent] Summary salvo na memoria de longo prazo.");
              } catch (memError) {
                // Tenta metodo alternativo
                try {
                  const { appendToMemory } = await import("../memory/memory_search");
                  appendToMemory(summary, "Historico Arquivado - " + this.name);
                  console.log("[Agent] Summary salvo via appendToMemory.");
                } catch (memError2) {
                  console.log("[Agent] Nao foi possivel salvar na memoria: " + (memError2 instanceof Error ? memError2.message : String(memError2)));
                }
              }
            }
          }
        } catch (e: any) {
          console.log("[Agent] Erro ao sumarizar historico: " + e.message);
          // Continua mesmo sem sumario - o truncamento e mais importante
        }

        // Substituir historico: system + ultimas mensagens
        this.history = [systemMsg, ...messagesToKeep];
        console.log("[Agent] Historico truncado para " + this.history.length + " mensagens.");
      }
  ...
  async run(userMessage: string, onProgress?: (msg: string) => Promise<void>): Promise<string> {
        ...
        // Trunca historico se necessario (com sumarizacao para memoria de longo prazo)
        await this.truncateHistoryWithSummary();
        try {
          while (true) {
            ...
            for (const toolCall of msg.tool_calls) {
              ...
              try {
                ...
                const errorMsg = `[Erro no parse do JSON dos argumentos da ferramenta '${toolName}']: ${e.message}. Corrija o JSON e tente novamente.`;
                console.warn(`[Agent] Erro ao parsear argumentos: ${e.message}`);
                this.history.push({
                  role: "tool" as const,
