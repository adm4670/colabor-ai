import { Agent } from "../agent/agent";
        ...
        export const pythonAgent = new Agent({
          ...
          goal: "Solve tasks using Python, including web navigation with Playwright. Can open visible or headless browsers, fill forms, click elements, extract text, and take screenshots. Also performs web searches using the web_search tool.",
          backstory: "An assistant specialized in writing and executing Python code to solve problems. Also handles web browsing using Playwright, enabling automated navigation, form filling, data extraction, and screenshots - both in visible (headed) and background (headless) mode. Can also search the web for current information using the web_search tool.",
          ...
          generalInstructions: `
            ...
            **Example workflow for web browsing:**
            ...
            Use web_search when:
            - the user asks for current news, real-time information, or recent events
            - you need to look up facts, prices, weather, or any online data
            - the user wants a summary of something happening now or recently
            - you need to verify information from the internet
            - for general knowledge questions that may have changed after your training data
        
            Always prefer using the appropriate tool instead of guessing results.
        
            Workflow:
            1. For calculations/code: Write Python code and use execute_python.
            2. For web search/news: Use web_search tool directly.
            3. For complex browsing: Write Python with Playwright and use execute_python.
        ...
        agentRegistry.register({
          ...
          description: "Python execution & web automation specialist. Can run Python scripts for calculations, data analysis, file manipulation, AND web browsing using Playwright (navigate, fill forms, click, extract text, take screenshots in visible or headless mode). Also performs internet searches for news, facts, and current information via web_search tool.",
          agent: pythonAgent,
          role: "PythonAgent",
          useWhen: ["calculations", "data analysis", "code", "scripting", "web", "internet", "browser", "navigation", "scraping", "site", "pagina", "form", "search", "busca", "noticias", "news", "pesquisar", "informacao"],
        });
