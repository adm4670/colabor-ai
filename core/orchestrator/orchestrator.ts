...
import { answerAgent as writerAgent } from "../agents/answer.agent";
...
export class AgentOrchestrator {
  ...
  async run(runInput: RunInput) {
    ...
    while (steps < maxSteps) {
      ...
      // === CONSTRUIR LISTA DE AGENTS COM HINTS ===
      // Tenta incluir useWhen hints se disponiveis (via AgentRegistry)
      let agentList = this.agents
    ...
    5. Use "browser" for web navigation, searching sites, scraping, or web automation.
    6. Use "python" for calculations, data analysis, or running Python code.
    7. Use "writer" to produce the final response shown to the user.
    
    === IMAGE AGENTS ===
    8. Use "image-reader" to ANALYZE and DESCRIBE existing images.
       - If the user sends an image (photo, picture) or asks "what's in this image", "describe this photo", "read this image" -> USE image-reader
       - If the user sent a photo in Telegram (indicated by [IMAGEM] in the request) -> USE image-reader
       - NEVER use assistant or browser for image analysis
    
    9. Use "image-generator" to CREATE and GENERATE new images.
       - If the user says "generate an image", "create a picture", "draw", "make a visual" -> USE image-generator
       - NEVER use assistant or python for image generation

    === KNOWLEDGE GRAPH AGENT ===
    10. Use "knowledge-graph" to analyze project architecture, scan code structure, map dependencies, and answer questions about the codebase.
    ...
    `;
      ...
      try {
        ...
        if (onProgress) {
          onProgress("Redirecionando para assistente...").catch(() => {});
        }
        parsed = { agent: "assistant", instruction: input };
      }
      ...
      if (parsed.agent === "finish") {
        ...
        appendToTranscript(this.sessionId, {
          role: "assistant",
          content: finalResponse || parsed.instruction || "Concluido.",
          timestamp: Date.now(),
        });
    ...
    appendToTranscript(this.sessionId, {
      role: "assistant",
      content: finalResponse || "Nao foi possivel concluir a tarefa.",
      timestamp: Date.now(),
    });
    ...
    this.eventStream.push(createEvent("agent_end"));
    this.eventStream.end(finalResponse as any);
    return finalResponse || "Nao foi possivel concluir a tarefa.";
  }
