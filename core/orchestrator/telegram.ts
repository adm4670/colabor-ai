...
import "../agents/browser.agent";   // ✅ CORRIGIDO: browser agent agora é importado e registrado
import "../agents/answer.agent";
// import "../agents/shell.agent";
import "../agents/task-manager.agent";
import "../agents/knowledge-graph.agent";
import "../agents/image-reader.agent";
import "../agents/image-generator.agent";
...
// ============================================================
// Utilitários de envio de mensagens Telegram
// ============================================================
...
function splitMessage(text: string, maxLen: number = 4000): string[] {
  ...
  while (remaining.length > 0) {
    ...
    logger ? logger.warn(`[Telegram] Mensagem forcadamente truncada em ${maxLen} caracteres`) : null;
  }
  return parts;
}
...
async function sendSafeMessage(
  ...
  if (parts.length > 1) {
    logger ? logger.info(`[Telegram] Mensagem dividida em ${parts.length} partes (${sanitized.length} chars)`) : null;
  }
  for (let i = 0; i < parts.length; i++) {
    ...
    try {
      if (useMarkdown) {
        try {
          ...
          if (parseErr?.message?.includes('parse') || parseErr?.description?.includes('parse')) {
            logger ? logger.warn(`[Telegram] Erro de parse na parte ${i+1}, reenviando sem formatacao`) : null;
      ...
      logger ? logger.error(`[Telegram] Falha ao enviar parte ${i + 1}/${parts.length}: ${err.message}`) : null;
      throw err;
    }
  }
}
...
async function startTelegramAgent() {
  ...
  startConsoleDashboard(orchestrator.eventStream);
  ...
  bot.on("message", async (msg) => {
    ...
    try {
      ...
      if (msg.photo) {
        ...
        const file = await bot.getFile(fileId);
        const url = `https://api.telegram.org/file/bot${process.env.TELEGRAM_TOKEN}/${file.file_path}`;
        text = `[IMAGEM] ${url} | ${msg.caption || "Descreva esta imagem em detalhes."}`;
      }

      // CASO 2: DOCUMENTO (imagem como arquivo)
      ...
      const onProgress = async (message: string) => {
        if (progressActive) {
          try {
            const thinkingMsg = await generateThinkingMessage({
              description: message,
            });
      ...
      if (progressShown && progressMsg) {
        try { await bot.deleteMessage(chatId, progressMsg.message_id); } catch { }
      }
